...
namespace CF.CameraBot
{
    ...
    public class MethodDrawer : PropertyDrawer
    {
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (property.isExpanded)
            {
				...
				if (prop.enumValueIndex > 0)
				{
					...
					EditorGUI.PropertyField(line, property.FindPropertyRelative("m_HorizontalPositionSpeed"));
					line = line.GetRectBottom().Adjust(y: lineS);
					EditorGUI.PropertyField(line, property.FindPropertyRelative("m_VerticalPositionSpeed"));
				}
		...
		public static float GetStaticHeight(SerializedProperty property)
		{
			...
			{
				rst += 150f + 32f + 16f;
				rst += (property.FindPropertyRelative("m_MoveMethod").enumValueIndex > 0 ? (lineH + lineS) * 2f : 0f);
