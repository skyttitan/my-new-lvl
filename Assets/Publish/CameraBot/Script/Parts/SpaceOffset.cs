...
namespace CF.CameraBot.Parts
{
	public class SpaceOffset : MonoBehaviour, IParts, IGizmo, IOffset
	{
		...
		{
			if(!transform.position.Approximately(transform.parent.position))
			{
				GizmosExtend.DrawLine(transform.position, transform.parent.position, m_Color.CloneAlpha(Mathf.PingPong(Time.time, m_Color.a)));
			}
		}
